...
2. **Dialog UI smokes** (FreeBSD VM) — `scripts/test-dialog-ui.sh` (cdialog
   widgets) plus one expect run (`scripts/test-dialog-expect.exp`: welcome +
   Ctrl+C). Interactive installs `pkg install cdialog` on first run if needed.
